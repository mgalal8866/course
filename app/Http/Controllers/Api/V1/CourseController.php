...
class CourseController extends Controller
{
            ...
            'childrens' => function ($q) use ($id) {
                $q->whereHas('courses', function ($qq) use ($id) {
                    ...
                    ->with(['lessons' => function ($qe) use ($id) {
                        $qe->wherePivot('course_id', $id)->orderBy('course_stages.id');
                    }]);
            },
            'childrens.lessons',
    ...
    public function get_calc_prog2(Request $request)
    {
            ...
            foreach ($Stages->childrens as $child) {
                ...
                if ($child->lessons->count() > 0) {
                    ...
                    foreach ($child->lessons as $les) {
                        ...
                        $allquiz_result_detailscount =   $q != null ? $q->quiz_result_details->count() : '0';
                        $questions['main'][count($questions['main']) - 1]['quiz'][count($questions['main'][count($questions['main']) - 1]['quiz'])]  =   [
                    ...
                    $total_degree = collect($questions['main'][count($questions['main']) - 1]['quiz'])->sum('degree');
        ...
        // $questions =  collect[ 'data'=> (($final_total_degree != 0) ? (($final_total_degree / $final_count_quiz) * 100) : 0)];

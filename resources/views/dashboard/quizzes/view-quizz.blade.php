                    ...
                    <ul class="nav nav-tabs" role="tablist" wire:ignore>
                        @foreach(\App\Enum\Quiz::cases() as $q)
                            <li class="nav-item" wire:ignore.self>
                                <a class="nav-link @if($q->value == $selecttab) active @endif" id="{{$q->value}}-tab" wire:click.prevent='changeselecttab({{$q->value }})' data-bs-toggle="tab"  aria-controls="tt{{$q->value}}" role="tab" aria-selected="true">{{ __('tran.typequiz-'.$q->name)}}</a>
                            </li>
                        @endforeach
                    </ul>

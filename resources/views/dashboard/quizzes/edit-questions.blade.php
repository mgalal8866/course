                ...
                @if ($question != null)
                    ...
                    <form id="editUserForm" class="row gy-1 pt-75" wire:submit.prevent="save">
                        <div class="col-4">
                            ...
                            <input class="form-control" wire:model="question.0.degree" type="number" step="0.1" />
                            @error('question.0.degree')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label" for="">وصف السؤال</label>
                            <x-summernote wire:model='question.0.description' id="description" />
                            @error('question.0.description')
                        ...
                        <div class="col-12">
                            ...
                            <x-summernote wire:model='question.0.question' id="question" />
                            @error('question.0.question')
                                <span class="error" style="color: red">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        @foreach ($question->answer as $key1 => $value1)
                            <div class="card shadow-none bg-transparent border-success">
                                <div class="card-header">
                                    <div class="form-check form-check-success">
                                        <input class="form-check-input" type="radio" name="question.0.answers"
                                            value="{{ $key1 }}" id="inlineRadio1"
                                            wire:model="question.0.correct" />
                                        <label class="form-check-label" for="inlineRadio1">الاجابه
                                            الصحيحة</label>
                                    </div>
                                    @error('question.0.correct')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="card-body">
                                    <div class="col-12">
                                        ...
                                        <x-summernote wire:model='question.0.answers.{{ $key1 }}.answer'
                                            id="answer_{{ $key1 }}" />
                                        @error('question.0.answers.' . $key1 . '.answer')
                                            <span class="error" style="color: red">{{ $message }}</span>
                                        @enderror
                                    </div>
                                </div>
